...
# Function Definition Self-Consistency CoT
def self_consistency_chain_of_thought(user_query:str, number_of_chains:int=4):
	# run CoT multiple times
	res = []
	for i in range(number_of_chains):
		res_chain = chain_of_thought(user_query=user_query)
		res.append(res_chain)
		print(f"{i+1}: {res_chain}")
		print("-"*90)
...
res = self_consistency_chain_of_thought(user_query=user_prompt, number_of_chains=4)
pprint(res)
